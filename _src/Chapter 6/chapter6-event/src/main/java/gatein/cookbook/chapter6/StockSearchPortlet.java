...
public class StockSearchPortlet extends GenericPortlet {
    ...
    public void addWatch(ActionRequest request, ActionResponse response) throws PortletException {
        ...
        for (String availStock : availableStocks) {
            if (availStock.startsWith(stock)) {
                String[] split = availStock.split(":");
                stockEvent.ticker(split[0]).name(split[1]);
                break;
            }
        }
        QName eventName = new QName("http://www.gatein.org/xml/ns/cookbook", "watchStockEvent");
        response.setEvent(eventName, stockEvent);
    }
